...
resource template identifiers are `ssh`, `mysql`, `postgresql`, `sqlserver`, `s3`, `minio`, `oss`,
`redis`, `elasticsearch`, `neo4j`, and `http`. Selecting a non-SSH template routes protected fields
